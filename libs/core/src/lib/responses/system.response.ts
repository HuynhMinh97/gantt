...
import { SysEntity, SysAllLangEntity } from '../entities/system.entity';
...
@ObjectType()
export class SystemAllLangResponse {
  @Field(() => [SysAllLangEntity], { nullable: true })
  data?: SysAllLangEntity[];
  ...
  constructor(status: number, result: SysAllLangEntity[], message: string) {
    this.status = status;
    switch (status) {
      case RESULT_STATUS.OK:
        this.data = result;
