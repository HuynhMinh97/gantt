...
export class AitDatePickerComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    for (const key in changes) {
      if (Object.prototype.hasOwnProperty.call(changes, key)) {
        ...
        if (key === 'format' && this.format) {
          const dateFormat = this.dateFormatService.formatDatePicker(
            this.dateInput,
            this.format
          );
  ...
  translateDate = (value: string) => {
    ...
    let res = null;
    const priorityDay =
      this.getFormat().indexOf('d') < this.getFormat().indexOf('M');
    const priorityYear =
      this.getFormat().indexOf('y') < this.getFormat().indexOf('M');
      ...
      if (value.length === 1) {
        ...
        if (target < lastDay) {
          ...
          if (priorityDay) {
            ...
            const c2_lastDay = this.getDaysInMonth(
              d_month,
              this.getCurrentYear()
            );
        ...
        if (!priorityDay) {
          ...
          const c3_lastDay = this.getDaysInMonth(
            c3_month,
            this.getCurrentYear()
          );
          ...
          const c3_e_lastDay = this.getDaysInMonth(
            c3_e_month,
            this.getCurrentYear()
          );
        ...
        if (!priorityDay) {
          ...
          if (c4_month > 12) {
            ...
            const c4_lastDay = this.getDaysInMonth(
              c4_month,
              this.getCurrentYear()
            );
        ...
        if (!priorityYear) {
          let ct_month = isNaN(Number(value[2] + value[3]))
            ? 0
            : Number(value[2] + value[3]);
          let ct_day = isNaN(Number(value[0] + value[1]))
            ? 0
          ...
          const ct_lastDay = this.getDaysInMonth(
            ct_month,
            this.getCurrentYear()
          );
          ...
          let ct_month = isNaN(Number(value[4] + value[5]))
            ? 0
            : Number(value[4] + value[5]);
          let ct_day = isNaN(Number(value[6] + value[7]))
            ? 0
            : Number(value[6] + value[7]);
  ...
  getFormat = (): string => {
    const format = this.disable
      ? this.formatDateTimeDisplay
      : this.formatDateTimeInput;
  ...
  isDateValid = (val) => {
    ...
    return (
      val instanceof Date &&
      !isNaN(val.valueOf()) &&
      this.dateFormatService.isRightFormatDate(val, this.getFormat())
    );
  };
  ...
  setupDate = () => {
    this.store.pipe(select(getSettingLangTime)).subscribe((setting) => {
      ...
      if (this.format) {
        if (
          AitAppUtils.isValidDate(this.dateInput) ||
          typeof this.dateInput === 'number'
        ) {
          if (this.dateInput) {
            let dateFormat;
            if ((this.disable || this.readonly) && !this.useFormat) {
              dateFormat = this.dateFormatService.formatDatePicker(
                this.dateInput,
                this.formatDateTimeDisplay
              );
              this.valueDf = dateFormat;
  ...
  ngOnInit() {
    if (this.defaultValue && this.defaultValue !== '') {
      ...
      this.watchValue.emit({
        value: target ? new Date(target).getTime() : target,
      });
    } else {
