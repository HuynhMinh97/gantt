...
@ObjectType()
export class RegisterProjectResponse {
  @Field(() => [RegisterProjectEntity], { nullable: true })
  data?: RegisterProjectEntity[];
  ...
  constructor(status: number, result: RegisterProjectEntity[], message: string) {
    this.status = status;
    switch (status) {
...
export class CandidateResponse {
  ...
  constructor(status: number, result: CandidateEntity[], message: string) {
    this.status = status;
    switch (status) {
      case RESULT_STATUS.OK:
        this.data = result;
