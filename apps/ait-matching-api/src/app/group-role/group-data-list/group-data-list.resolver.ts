...
import { GroupDataListRequest } from './group-data-list.request';
import { GroupDataListResponse } from './group-data-list.response';

@Resolver()
export class GroupDataListResolver extends AitBaseService {
  @Query(() => GroupDataListResponse, { name: 'getAllRole' })
  async getAllRole(
    @AitCtxUser() user: SysUser,
    @Args('request', { type: () => GroupDataListRequest })
    request: GroupDataListRequest
  ) {
    const aqlQuery = `
    FOR v IN  sys_role
    RETURN v
    ...
    const response = new GroupDataListResponse(
      200,
      groupDataList as GroupDataListEntity[],
      ''
    );
