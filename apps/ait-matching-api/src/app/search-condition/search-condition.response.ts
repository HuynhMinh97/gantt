...
@ObjectType()
export class SearchConditionResponse {
  @Field(() => [SearchConditionEntity], { nullable: true })
  data?: SearchConditionEntity[];
  ...
  constructor(status: number, result: SearchConditionEntity[], message: string) {
    this.status = status;
    switch (status) {
